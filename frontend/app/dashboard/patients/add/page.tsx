...
export default function AddPatientPage() {
  ...
  const handleNextStep = async (nextStep: number) => {
    if (!isStepValid(currentStep)) { alert("Veuillez remplir correctement tous les champs obligatoires avant de continuer."); return; }
    if (nextStep === 8) {
      // Afficher immédiatement l’étape CDSS : l’appel réseau peut prendre
      // quelques secondes et ne doit pas laisser l’utilisateur sur l’étape 7.
      setCurrentStep(8);
      setMaxReachedStep(8);
      setLoading(true);
      setError(null);
    ...
    setCurrentStep(nextStep); setMaxReachedStep((old) => Math.max(old, nextStep));
  };
  const handleFinal = () => { if (!createdIds) return; alert(`Patient enregistré. ID patient : ${createdIds.id_patient}`); };
  const steps = ["Démographie", "Clinique", "Biologie", "Imagerie", "TNM", "Histologie", "Parcours", "Calculs CDSS", "Évaluation"];

  return <div className="max-w-5xl mx-auto space-y-6 font-serif">
    <div className="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm flex items-center justify-between text-xs overflow-x-auto">{steps.map((label, index) => { const step = index + 1; const completed = isStepValid(step) && maxReachedStep > step; const accessible = step <= 7 ? canAccessStep(step) : step <= maxReachedStep; return <div key={step} className="flex items-center flex-1 min-w-max"><button type="button" onClick={() => accessible && setCurrentStep(step)} disabled={!accessible} className={`flex items-center gap-2 font-bold transition ${!accessible ? "cursor-not-allowed opacity-40" : "cursor-pointer"} ${currentStep === step ? "text-[#1D7893]" : "text-gray-400"}`}><span className={`w-7 h-7 rounded-full flex items-center justify-center text-white ${currentStep === step ? "bg-[#1D7893]" : completed ? "bg-emerald-500" : "bg-gray-300"}`}>{completed ? "✓" : step}</span><span>{label}</span></button>{step < steps.length && <div className="h-[2px] bg-gray-200 flex-1 mx-2 min-w-[15px]" />}</div>; })}</div>
    ...
    {currentStep === 9 && <div className="space-y-6"><EvaluationPage /><div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm flex justify-between items-center"><button type="button" onClick={() => setCurrentStep(8)} className="flex items-center gap-2 px-5 py-2.5 rounded-xl border border-gray-300 text-gray-700 text-xs font-semibold hover:bg-gray-50"><ArrowLeft className="w-4 h-4" /> Modifier les calculs</button><button type="button" onClick={handleFinal} disabled={!createdIds} className="flex items-center gap-2 px-8 py-3 rounded-xl bg-emerald-600 hover:bg-emerald-700 disabled:opacity-50 text-white text-xs font-bold shadow-lg"><CheckCircle2 className="w-5 h-5" /> Enregistrer définitivement</button></div></div>}
  </div>;
}
