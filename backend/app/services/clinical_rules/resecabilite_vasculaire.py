"""
Moteur de déduction de la résécabilité vasculaire — Adénocarcinome du pancréas
===============================================================================

Référence clinique unique et traçable :
    TNCD (Thésaurus National de Cancérologie Digestive), Chapitre 9
    "Cancer du pancréas", version du 17/05/2024, section 9.2.5, Tableau IV
    "Définition des critères de résécabilité selon le stade d'envahissement
    vasculaire" — classification NCCN 2015 / consensus international
    Isaji et al., 2018.

Portée de ce module
---------------------------------------------
Ce moteur calcule le champ pivot `resecabilite_anatomique` en 3 étapes,
reprises du document CDSS (Partie A.4, "Note technique") et du TNCD :

    Étape 1+2 : classification par vaisseau (AMS, TC, AHC, VMS/VP), puis
                statut le plus péjoratif des 4 -> Tableau IV, TNCD 9.2.5.
    Étape 3   : correction finale si `metastases=True` (bascule branche
                métastatique) ou `adenopathie_distance=True` (force
                LOCALEMENT_AVANCE) -> TNCD 9.2.5 p.15 pour l'adénopathie à
                distance ; structure TNM (Tableau III) pour le statut M.
                L'adénopathie RÉGIONALE (loge pancréatique) est volontairement
                exclue de cette correction : ce n'est pas la même donnée.

Il ne prend PAS en compte :
- le CA19-9 (critère "B" biologique, seuil > 500 U/mL) ;
- l'état général OMS (critère "C" clinique, PS ≥ 2) ;
  -> ces deux critères relèvent du module `criteres_abc` (autre tâche).

Note : la correction de l'étape 3 repose sur un principe clinique explicite
du TNCD (9.2.5) et ne dépend pas de la formulation exacte d'une future
recommandation textuelle (ex. Règle T2 d'un éventuel moteur de règles) —
ce module renvoie un STATUT, jamais un texte de recommandation.

Le Tableau IV décrit 4 vaisseaux, chacun avec sa propre logique :

    Vaisseau     | Résécable        | Borderline                          | Localement avancé
    -------------|------------------|--------------------------------------|--------------------------------
    AMS          | Pas de contact   | Contact < 180°                       | Contact ≥ 180°
    Tronc cœliaque (TC) | Pas de contact | Contact < 180°                 | Contact ≥ 180°
    AHC          | Pas de contact   | Contact court, sans extension au TC   | Chirurgicalement non
                 |                  | ou à l'origine de l'AHC,               | reconstructible
                 |                  | chirurgicalement reconstructible      |
    VMS/VP       | Contact < 180°   | Contact ≥ 180° OU (contact < 180°     | Occlusion tumorale
                 | sans irrégularité| avec irrégularité de calibre sans     | chirurgicalement NON
                 | de calibre       | occlusion) OU occlusion               | reconstructible OU
                 |                  | reconstructible                       | envahissement des veines
                 |                  |                                        | jéjunales principales

Statut global = le plus péjoratif des 4 statuts vaisseau (une seule atteinte
localement avancée rend l'ensemble localement avancé), sauf que des données
manquantes (INDETERMINE) ne peuvent jamais "améliorer" un résultat déjà
Localement Avancé -> ordre de priorité explicite (voir _PRIORITE).

Ce module est un objet métier pur (aucun accès DB, aucun appel réseau) afin
de rester testable unitairement et indépendant du schéma exact des tables
`imagerie_vasculaire` / `regle_decision` / `recommandation_generee`.
L'adaptation "ligne DB -> ImagerieVasculaireInput" doit se faire dans la
couche API (voir exemple d'intégration en bas de fichier).
"""
