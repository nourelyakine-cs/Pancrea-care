...
def list_evaluations(db: Session, id_dossier: int) -> list[EvaluationClinique]:
    dossier = db.get(DossierPatient, id_dossier)
    if dossier is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
...
def get_evaluation(db: Session, id_evaluation: int) -> EvaluationClinique:
...
def get_biologie(db: Session, id_evaluation: int) -> Biologie:
    get_evaluation(db, id_evaluation)
    return (
        db.query(Biologie).filter(Biologie.id_evaluation == id_evaluation).first()
    )
...
def upsert_biologie(
    ...
    if biologie is None:
        biologie = Biologie(id_evaluation=id_evaluation, **payload.model_dump())
        db.add(biologie)
    ...
    log_action(
        ...
        detail={"id_evaluation": id_evaluation, **payload.model_dump()},
    )
    db.commit()
    db.refresh(biologie)
    return biologie
...
def delete_biologie(db: Session, id_evaluation: int, medecin_id: int | None = None) -> None:
    get_evaluation(db, id_evaluation)
    biologie = get_biologie(db, id_evaluation)
    if biologie is None:
        raise HTTPException(
...
def get_histologie(db: Session, id_evaluation: int) -> HistologieBiologie:
    get_evaluation(db, id_evaluation)
    return (
...
def upsert_histologie(
    ...
    if histologie is None:
        histologie = HistologieBiologie(id_evaluation=id_evaluation, **payload.model_dump())
        db.add(histologie)
...
def delete_histologie(db: Session, id_evaluation: int, medecin_id: int | None = None) -> None:
    get_evaluation(db, id_evaluation)
    histologie = get_histologie(db, id_evaluation)
    if histologie is None:
        raise HTTPException(
...
def list_imageries(db: Session, id_evaluation: int) -> list[Imagerie]:
    get_evaluation(db, id_evaluation)
    return (
        db.query(Imagerie)
...
def create_imagerie(
    ...
    get_evaluation(db, id_evaluation)
    imagerie = Imagerie(id_evaluation=id_evaluation, **payload.model_dump())
    db.add(imagerie)
    db.flush()
    log_action(
        db,
...
def get_imagerie(db: Session, id_imagerie: int) -> Imagerie:
...
def delete_imagerie(db: Session, id_imagerie: int, medecin_id: int | None = None) -> None:
    imagerie = get_imagerie(db, id_imagerie)
    log_action(
        db,
        medecin_id=medecin_id,
...
def list_metastases(db: Session, id_imagerie: int) -> list[MetastaseLocalisation]:
...
def create_metastase(
    ...
    get_imagerie(db, id_imagerie)
    metastase = MetastaseLocalisation(id_imagerie=id_imagerie, **payload.model_dump())
    db.add(metastase)
    try:
        db.flush()
    except IntegrityError:
...
def get_metastase(db: Session, id_metastase: int) -> MetastaseLocalisation:
...
def delete_metastase(db: Session, id_metastase: int, medecin_id: int | None = None) -> None:
    metastase = get_metastase(db, id_metastase)
    log_action(
        db,
        medecin_id=medecin_id,
...
def list_analyses(db: Session, id_evaluation: int) -> list[AnalyseMoleculaire]:
    get_evaluation(db, id_evaluation)
    return (
...
def create_analyse(
    ...
    get_evaluation(db, id_evaluation)
    analyse = AnalyseMoleculaire(id_evaluation=id_evaluation, **payload.model_dump())
    db.add(analyse)
    db.flush()
    log_action(
        db,
...
def get_analyse(db: Session, id_analyse: int) -> AnalyseMoleculaire:
...
def delete_analyse(db: Session, id_analyse: int, medecin_id: int | None = None) -> None:
    analyse = get_analyse(db, id_analyse)
    log_action(
        db,
        medecin_id=medecin_id,
...
def list_comorbidites(db: Session, id_evaluation: int) -> list[EvaluationComorbidite]:
    get_evaluation(db, id_evaluation)
    return (
